...
int MJhand::get_tile_suit(int _ID)const{
	return _tiles[_ID+_faceup_len-1].suit();
}
int MJhand::get_tile_rank(int _ID)const{
	return _tiles[_ID+_faceup_len-1].rank();
}

int MJhand::get_tile_suit_z(int _ID)const{
	return _tiles[_ID].suit();
} // count from 0

int MJhand::get_tile_rank_z(int _ID)const{
	return _tiles[_ID].rank();
} // count from 0
...
vector<MJtile> MJhand::eat(const MJtile& t, int method){
    vector<MJtile> vm;
    vm.clear();
    int _c_caneat=caneat(t);
    
    if(method==3)method=4; // 100
    if(_c_caneat==0)return vm; // cannot eat
    if(t.suit()==4)return vm; // wind or flower
    
    if(((_c_caneat & 1) && (_c_caneat & 2)) ||
       ((_c_caneat & 2) && (_c_caneat & 4)) ||
       ((_c_caneat & 4) && (_c_caneat & 1))){
        // at least two methods
        if((method & _c_caneat)==0)return vm;
    }
    else {
        method=_c_caneat;
    }
    ...
    if(method==1){
        vm.push_back(MJtile(t.getTileId()-2*16));
        vm.push_back(MJtile(t.getTileId()));
        vm.push_back(MJtile(t.getTileId()-1*16));
    }
    else if(method==2){
        vm.push_back(MJtile(t.getTileId()-1*16));
        vm.push_back(MJtile(t.getTileId()));
...
vector<MJtile> MJhand::pong(const MJtile& _draw){
    vector<MJtile> vm;
    vm.clear();

    if(canpong(_draw) == false)return vm;
    ...
    for(int i=0;i<3;++i)vm.push_back(_draw.getTileId());
    return vm;
}

vector<MJtile> MJhand::minggone(const MJtile& _draw, MJcollection& _collection){
    vector<MJtile> vm;
    vm.clear();

    if(canminggone(_draw) == false)return vm;
    ...
    for(int i=0;i<4;++i)vm.push_back(_draw.getTileId());
    return vm;
}

vector<MJtile> MJhand::angone(int index, MJcollection& _collection){
    vector<MJtile> vm;
    vm.clear();

    if(index>_total_len+_stage-_faceup_len || index<1)
        return vm; // index isn't correct
    
    MJtile t = _tiles[_faceup_len+index-1];
    
    if(canangone(t)==false)
        return vm; // can't angone
    ...
    //for(int i=0;i<4;++i)vm.push_back(t.getTileId());

    return vm;
}

vector<MJtile> MJhand::bugone(int index, MJcollection& _collection){
    vector<MJtile> vm;
    vm.clear();
...
ostream& operator << (ostream& os, MJhand& h){
    int _if_hu=0;
	if(h._stage==2){
		_if_hu=1;
		h._stage=0;
